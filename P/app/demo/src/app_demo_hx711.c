...
// 修改后的hi_hx711_task_1函数，解决weight2显示不出数据的问题
...
hi_void hi_hx711_task_1(hi_void)
{
    ...
    // 2号秤
    hi_io_set_func(HI_IO_NAME_GPIO_3, HI_IO_FUNC_GPIO_3_GPIO); // 数据
    hi_gpio_set_dir(HI_GPIO_IDX_3, HI_GPIO_DIR_IN);
    hi_io_set_func(HI_IO_NAME_GPIO_4, HI_IO_FUNC_GPIO_4_GPIO); // 时钟
    hi_gpio_set_dir(HI_GPIO_IDX_4, HI_GPIO_DIR_OUT);
    hi_gpio_set_ouput_val(HI_IO_NAME_GPIO_4, 0);

    oled_show_str(0, 1, "Weight1 : ", 1);
    oled_show_str(0, 5, "Weight2 : ", 1);

    double base_data_1 = 0;
    double hx711_weight_1 = 0;
    char buf[21];
    base_data_1 = Get_Sensor_Read_1(); // 获取基准值
    double base_data_2 = 0;
    ...
    while (1)
    {
        hx711_weight_1 = ((Get_Sensor_Read_1() - base_data_1) / GapValue_1); // 获取重量
        hx711_weight_2 = ((Get_Sensor_Read_2() - base_data_2) / GapValue_2); // 获取重量
        memset(buf, 0, sizeof(buf));

        if (hx711_weight_1 > 0)
        {
            sprintf_s(buf, sizeof(buf), "%.2f g   ", hx711_weight_1);
            oled_show_str(20, 3, buf, 1);
        }
        else
        {
            double temp_weight = -hx711_weight_1; // 使用一个临时变量来避免直接修改原值
            sprintf_s(buf, sizeof(buf), "%.2f g   ", temp_weight);
            oled_show_str(20, 3, buf, 1);
        }
        memset(buf, 0, sizeof(buf));
        ...
        if (hx711_weight_2 > 0)
        {
            sprintf_s(buf, sizeof(buf), "%.2f g   ", hx711_weight_2);
            oled_show_str(20, 7, buf, 1);
        }
        else
        {
            double temp_weight = -hx711_weight_2; // 使用一个临时变量来避免直接修改原值
            sprintf_s(buf, sizeof(buf), "%.2f g   ", temp_weight);
            oled_show_str(20, 7, buf, 1);
        }
        ...
        if (W_Flag_2 && (hx711_weight_2 > Weigth_2_Flag - 30))
        {
            W_Flag_2 = 0;
            // 关闭舵机
        ...
        else if (hx711_weight_2 <= Weigth_2_Flag - 30)
        {
            W_Flag_2 = 1;
        }
        hi_sleep(100);
