...
/**
	@berf read  th06 serson data
	@param hi_i2c_idx id：Select I2C channel
	@param hi_u32 recv_len：Read data length
	@param hi_u8 type：Read data type（TEMPERATURE/HUMIDITY）
...
hi_float th06_read(hi_i2c_idx id, hi_u32 recv_len, hi_u8 type)
{
    ...
    if (type == TEMPERATURE) {
        temper = (recv_data[0]<<8 | recv_data[1]);//Temperature splicing
        Tem = (175.72*(float)temper/65536.0-46.85);   // T= -46.85+175.72*temper/(2^16-1)
        return Tem;

    } else if (type == HUMIDITY) {
        humi = (recv_data[0]<<8 | recv_data[1]);//Humidity splicing
...
hi_void app_demo_th06_task(hi_void)
{
    ...
    attr.stack_size = TH06_DEMO_TASK_STAK_SIZE;//Task stack memory
    attr.task_prio = TH06_DEMO_TASK_PRIORITY;//The task priority ranges from 0 to 31. Tasks 0 to 10 should not be used. The SDK has been used. The higher the value, the lower the priority
    attr.task_name = (hi_char*)"app_demo_th06_task";//Task name
	/*create task*/
